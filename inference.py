    ...
    if st.button("🎲 Lấy ngẫu nhiên"):
        ...
        for c_name in CLASS_NAMES:
            ...
            if os.path.exists(c_dir):
                images = glob.glob(os.path.join(c_dir, "*.*"))
                all_images.extend([(img, c_name) for img in images])
        if all_images:
            chosen_img, t_label = random.choice(all_images)
            st.session_state['random_img'] = chosen_img
            st.session_state['true_label'] = t_label
...
if uploaded_file is not None and not st.session_state.get('use_random', False):
    ...
    original_image = Image.open(st.session_state['random_img']).convert("RGB")
